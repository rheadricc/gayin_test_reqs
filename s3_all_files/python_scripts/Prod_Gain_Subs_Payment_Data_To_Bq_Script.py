    ...
    for field in array_fields:
        if field not in json_data:
            if field == "countries":
                ...
                adjusted_data[field] = [{"promotionId": None, "type": None, "isActive": None, "name": None, "code": None, "applyDate": None, "leaveDate": None, "benefits": [{"isFreePremium": None, "currency": None, "discountPeriod": None, "fixedPrice": None, "freePremiumByDay": None, "freePremiumByMonth": None, "usedPeriod": None}]}]
        ...
        if isinstance(arr, list):
            if len(arr) == 0:
                ...
                if field_name == "countries":
                    ...
                    return [{"promotionId": None, "type": None, "isActive": None, "name": None, "code": None, "applyDate": None, "leaveDate": None, "benefits": [{"isFreePremium": None, "currency": None, "discountPeriod": None, "fixedPrice": None, "freePremiumByDay": None, "freePremiumByMonth": None, "usedPeriod": None}]}]
...
def move_to_fail_path(bucket_name, file_name, fail_base_path, **context):
    ...
    try:
        ...
        if not match:
            raise ValueError(f"Prod_Gain_Subs_Pay_S3_To_Bq, dosya ismi beklenen formatta değil: {file_name}")
        ...
        logger.warning(f"❗ Prod_Gain_Subs_Pay_S3_To_Bq, Dosya fail klasörüne taşındı: {fail_path}")

    except Exception as e:
        msg = f"❌ Prod_Gain_Subs_Pay_S3_To_Bq, Fail klasörüne taşıma hatası: {file_name}\nHata: {e}"
        logger.error(msg)
        send_teams_alert(msg, context)
        send_slack_message(msg, context)
