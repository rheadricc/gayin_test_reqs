...
UpdateData as (
    ...
    FROM
        ...
        FROM "gain-dwh-prod"."int_transaction"."subs_payment"
            WHERE DATE(created_At) > '2025-02-03' and DATE(created_At) <= CURRENT_DATE - 1
        ) WHERE rownum = 1
),
ReportData as (
SELECT
...
(
    ...
    FROM ReportData
    where DATE(grace_until) >= CURRENT_DATE - 1
        and status = 'IN_GRACE'
        and subscription_plan_id is not null
),
...
(
    ...
    FROM ReportData
        where date(created_at) = CURRENT_DATE - 1
            and date(free_trial_start_date) = CURRENT_DATE - 1
...
(
    ...
    FROM ReportData
        where CURRENT_DATE - 1 <= valid_until
            and CURRENT_DATE - 1 >= date(free_trial_start_date) and CURRENT_DATE - 1 <= date(free_trial_end_date)
            and DATEDIFF(days,free_trial_start_date::timestamp,valid_until::timestamp) = 7
            and status = 'ACTIVE'
),
...
(
    ...
    FROM ReportData
where status = 'ACTIVE'
    and DATE(JSON_EXTRACT_PATH_TEXT(JSON_EXTRACT_ARRAY_ELEMENT_TEXT(applied_promotions,0),'applyDate')) = CURRENT_DATE - 1
...
where --status = 'ACTIVE' and
    DATEADD(day,7,DATE(JSON_EXTRACT_PATH_TEXT(JSON_EXTRACT_ARRAY_ELEMENT_TEXT(applied_promotions,0),'applyDate'))) >= CURRENT_DATE - 1
    and JSON_EXTRACT_PATH_TEXT(JSON_EXTRACT_ARRAY_ELEMENT_TEXT(JSON_EXTRACT_PATH_TEXT(JSON_EXTRACT_ARRAY_ELEMENT_TEXT(applied_promotions,0),'benefits'),0),'freePremiumByDay') = 7
),
