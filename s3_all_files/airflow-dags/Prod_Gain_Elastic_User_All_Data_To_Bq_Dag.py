from airflow import DAG
from airflow.operators.python import PythonOperator
from datetime import datetime, timedelta
import boto3
import os
import tempfile
import importlib.util
import sys

def download_and_import_function(bucket_name, s3_key, module_name='dynamic_user_script'):
    s3 = boto3.client('s3')
    temp_dir = tempfile.gettempdir()
    local_file_path = os.path.join(temp_dir, f"{module_name}.py")
    s3.download_file(bucket_name, s3_key, local_file_path)
    print(f"📥 S3'ten modül indirildi: {local_file_path}")
    spec = importlib.util.spec_from_file_location(module_name, local_file_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    return module

default_args = {
    "owner": "airflow",
    "depends_on_past": False,
    "start_date": datetime(2025, 4, 24),
    "retries": 1,
    "retry_delay": timedelta(minutes=5),
}

with DAG(
    dag_id="Prod_Gain_Elastic_User_All_Data_To_Bq_Dag",
    default_args=default_args,
    schedule_interval="30 3 * * *",
    catchup=True,
    max_active_runs=1,
    tags=["elastic", "user", "all_data", "scd"],
) as dag:

    def run_fetched_script(**kwargs):
        execution_date = kwargs["execution_date"]
        if isinstance(execution_date, str):
            import pandas as pd
            execution_date = pd.to_datetime(execution_date)

        module = download_and_import_function(
            bucket_name="gain-data-airflow-bucket",
            s3_key="python_scripts/Prod_Gain_Elastic_User_All_Data_To_Bq_Script.py"
        )
        module.fetch_dim_user_raw_and_push_bq(execution_date=execution_date)

    run_etl_task = PythonOperator(
        task_id="fetch_and_insert_dim_user_raw",
        python_callable=run_fetched_script,
        provide_context=True,
    )

    run_etl_task
