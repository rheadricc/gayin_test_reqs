...
default_args = {
    ...
    'start_date': datetime(2025, 1, 23, 10),
    'retries': 1,
...
def handle_failed_load(bucket_name, fail_base_path, **context):
    ...
    query = f"""
        ...
        WHERE database_name = 'gain-dwh-prod'
        AND table_id = 340647
    ...
    while True:
        ...
        try:
            if check_s3_path_exists(bucket_name, s3_path):
                copy_s3_prod_pay_subs_to_redshift = S3ToRedshiftOperator(
                    task_id='copy_s3_prod_pay_subs_to_redshift',
                    schema='int_transaction',
                    table='subs_payment',
                    s3_bucket=bucket_name,
                    s3_key=s3_path,
                    copy_options=["FORMAT AS JSON 's3://json-path-file-bucket/prod_subs_pay_json_pathfile.json'", "TIMEFORMAT 'auto'",],
                    aws_conn_id='aws_default',
                    redshift_conn_id='Redshift_Serverless_Prod_User',
                    method='APPEND',
                )
                copy_s3_prod_pay_subs_to_redshift.execute(context=context)
            ...
            message = f"Prod-Subs-Payment verisi yükleme sırasında hata oluştu: {str(e)}"
            #print(message)
...
with DAG(
    ...
    description='Transfer data from S3 prod_subs_pay to Redshift table gain-dwh-prod.int_transaction.subs_payment',
    ...
    delete_old_data_task = PostgresOperator(
        ...
        postgres_conn_id='Redshift_Serverless_Prod_User',
        sql="""
        DELETE FROM int_transaction.subs_payment
    ...
    update_inserted_date_task = PostgresOperator(
        ...
        postgres_conn_id='Redshift_Serverless_Prod_User',
        sql="""
        UPDATE int_transaction.subs_payment
