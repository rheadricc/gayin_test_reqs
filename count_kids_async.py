...
async def main():
    ...
    async with aiohttp.ClientSession(
        ...
        for page in range(start_page, pages_to_scan + 1):
            ...
            # normal akış: ids'leri işle
            s, k = await process_ids(session, ids)   # sende hangi fonksiyonsa
            scanned_total += s
            kids_total += k
        ...
        for round_no in range(1, FAILED_RETRY_ROUNDS + 1):
            ...
            for page in failed_pages:
                ...
                s, k = await process_ids_parallel(ids, f"FAILED page {page}")
                scanned_total += s
                kids_total += k
