...
--   - PREPAID excluded
--   - April monthly net revenue / active subscribers / ARPU by payment option
--   - month-end MRR on last day of month

WITH params AS (
  SELECT
    DATE '2026-04-01' AS ds_start,
    DATE '2026-04-30' AS ds_end
),
...
payment_option_config AS (
  ...
  SELECT 'IYZICO'          AS payment_option, 0.029 AS commission_rate
),
...
monthly_kpis AS (
  ...
  GROUP BY month, payment_option
),

mrr_eom_monthly AS (
  SELECT
    DATE_TRUNC(a.dt, MONTH) AS month,
    a.payment_option,
    SUM(
      a.amount_gross_tl
      * (1.0 - COALESCE(c.commission_rate, 0.00))
  ...
  LEFT JOIN payment_option_config c
    ON a.payment_option = c.payment_option
  WHERE a.dt = (SELECT ds_end FROM params)
  GROUP BY month, a.payment_option
)
...
SELECT
  ...
  m.mrr_eom_tl
FROM monthly_kpis k
LEFT JOIN mrr_eom_monthly m
  ON k.month = m.month
 AND k.payment_option = m.payment_option
ORDER BY k.month, k.payment_option;