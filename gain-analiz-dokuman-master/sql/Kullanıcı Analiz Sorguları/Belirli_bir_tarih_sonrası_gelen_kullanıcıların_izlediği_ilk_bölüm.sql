...
-- Belirli_bir_tarih_sonrası_gelen_kullanıcıların_izlediği_ilk_bölüm
-- ===============================================================
WITH 
...
-- 4️⃣ contentsnew: İçerik ve bölüm bilgilerini Backoffice metadata'dan al
contentsnew AS (
  SELECT 
    season_info,
    titleid,
    REPLACE(displayname,'"', '') AS displayname,
    TRIM(CONCAT(REPLACE(displayname,'"', '')," ",IF(season_info = 'VİDEO TANIMI','',season_info))) AS playlistid,
    JSON_VALUE(video, '$.videoContentId') AS video_id,
    JSON_VALUE(video, '$."name.tr-tr"') AS video_name,
    JSON_VALUE(video, '$.type') AS video_type,
    ...
    PARSE_DATE('%Y',CAST(publishyear AS STRING)) AS pubdate,
    '' AS modifiedAt,
    '' AS is_local_content
...
-- 5️⃣ first_event: Her kullanıcı için en erken izleme zamanını al
first_event AS (
  SELECT
    user_id,
...
-- 6️⃣ first_watched: Kullanıcıların ilk izlediği içerik ve bölüm
first_watched AS (
  SELECT DISTINCT
    a.user_id,
    a.datetime_ist,
  ...
  JOIN first_event f 
    ON a.user_id = f.user_id AND a.datetime_ist = f.min_event_time
  JOIN contentsnew c 
    ON a.video_id = c.video_id
  WHERE event_date >= '2025-07-16'
)
-- 7️⃣ Ana çıktı: Kullanıcı sayısı ve ilk izledikleri içerik ve bölüm
SELECT
  COUNT(DISTINCT rd.user_id) AS cnt_dst,  -- Belirli tarih sonrası aktif kullanıcı sayısı
  fw.playlistid,  -- İlk izlenen playlist
  fw.video_name   -- İlk izlenen bölüm
FROM ReportData rd 
...
GROUP BY fw.playlistid, fw.video_name
